...
class Student_model extends MY_Model
{
    ...
    public function getByStudentSession($student_session_id)
    {
        ...
        $this->db->join('school_houses', 'school_houses.id = students.school_house_id', 'left');
        $this->db->join('users', 'users.user_id = students.id', 'left');
        $this->db->join('custom_field_values as payment_mode', 'students.id = payment_mode.belong_table_id AND payment_mode.custom_field_id = 1', 'left');
        $this->db->join('custom_field_values as location', 'students.id = location.belong_table_id AND location.custom_field_id = 2', 'left');
        $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('users.role', 'student');
    ...
    public function getDatatableByClassSection($class_id = null, $section_id = null, $from_date = null, $to_date = null)
    {
        $this->datatables
            ->select('classes.id as `class_id`, student_session.id as student_session_id, students.id, classes.class, sections.id as `section_id`, sections.section, students.id, students.admission_no, students.roll_no, students.admission_date, students.firstname, students.middlename, students.lastname, students.image, students.mobileno, students.email, students.state, students.city, students.pincode, students.religion, students.dob, students.current_address, students.permanent_address, IFNULL(students.category_id, 0) as `category_id`, IFNULL(categories.category, "") as `category`, students.adhar_no, students.samagra_id, students.bank_account_no, students.bank_name, students.ifsc_code, students.guardian_name, students.guardian_relation, students.guardian_phone, students.guardian_address, students.is_active, students.created_at, students.updated_at, students.father_name, students.app_key, students.parent_app_key, students.rte, students.gender ,installment_due_date.due_date as installment_due_date,payment_mode.field_value as payment_mode,location.field_value as location,student_fees_deposite.amount_detail')
            ...
            ->join('installment_due_date', 'installment_due_date.student_fees_master_id = student_fees_master.id')
            ->join('categories', 'students.category_id = categories.id', 'left')
            ->join('custom_field_values as payment_mode', 'students.id = payment_mode.belong_table_id AND payment_mode.custom_field_id = 1', 'left')
            ->join('custom_field_values as location', 'students.id = location.belong_table_id AND location.custom_field_id = 2', 'left')
        ...
        if ($from_date != null) {
            $formatted_from_date = date('Y-m-d 00:00:00', strtotime($from_date));
            $this->datatables->where('installment_due_date.due_date >=', $formatted_from_date);
        }

        if ($to_date != null) {
            $formatted_to_date = date('Y-m-d 23:59:59', strtotime($to_date));
            $this->datatables->where('installment_due_date.due_date <=', $formatted_to_date);
        }
    ...
    public function getDatatableByFullTextSearch($searchterm)
    {
        $userdata            = $this->customlib->getUserData();
        $class_section_array = $this->customlib->get_myClassSection();
        $this->datatables->select('`classes`.`id` as `class_id`,`students`.`id`,`student_session`.`id` as `student_session_id`,`classes`.`class`,sections.id as `section_id`,sections.section,students.id,students.admission_no, students.roll_no,students.admission_date,students.firstname,students.middlename,students.lastname,students.image,  students.mobileno,students.email ,students.state,students.city,students.pincode,students.religion,students.dob ,students.current_address,students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name,students.ifsc_code ,students.father_name,students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.gender,students.rte,student_session.session_id,installment_due_date.due_date as installment_due_date,payment_mode.field_value as payment_mode,location.field_value as location,student_fees_deposite.amount_detail');
        ...
        $this->datatables->join('student_fees_deposite', 'student_fees_deposite.student_fees_master_id=student_fees_master.id');
        $this->datatables->join('fee_groups_feetype', 'fee_groups_feetype.id=student_fees_deposite.fee_groups_feetype_id');
        $this->datatables->join('installment_due_date', 'installment_due_date.feetype_installment_id=fee_groups_feetype.feetype_id','LEFT');
        ...
        $this->datatables->join('custom_field_values as payment_mode', 'students.id = payment_mode.belong_table_id AND payment_mode.custom_field_id = 1', 'left');
        $this->datatables->join('custom_field_values as location', 'students.id = location.belong_table_id AND location.custom_field_id = 2', 'left');
